...
public:
  ...
  /**
   * Returns this timestamp as the number of seconds in fixed length string
   * format
   */
  string getStringTime() const;

  /**
   * Returns the difference in seconds between this timestamp (greater) and t
   * (smaller) If the order is swapped, a negative number is returned
