...
/**
    * @brief 构造函数的主要功能是初始化 ImuCamPose
对象，通过读取关键帧中的IMU和相机位姿信息，将这些数据存储到内部成员变量中。它根据相机数量动态分配内存，设置各个相机的平移、旋转以及IMU的相对位姿
    * @param pKF KeyFrame pointer
    * @param num_cams number of cameras
    * @param tcw translation of camera w.r.t. world
    * @param Rcw rotation of camera w.r.t. world
    * @param tcb translation of camera w.r.t. body
    * @param Rcb rotation of camera w.r.t. body
    * @param tbc translation of body w.r.t. camera
    * @param Rbc rotation of body w.r.t. camera
    * @param pCamera pointer to camera object
    * @param Rwb0 rotation of imu w.r.t. body at time 0
    * @param DR delta rotation between imu and body at time 0
    * @param twb translation of imu w.r.t. body at time 0
    * @param Rwb rotation of imu w.r.t. body at time 0
    * @param its Imu-specific information
    * @note This constructor is used for posegraph optimization with 6DoF
    * @note The constructor assumes that the IMU is static and the camera is
fixed
*/
ImuCamPose::ImuCamPose(KeyFrame *pKF) : its(0) {
  ...
  if (num_cams > 1) {
    Eigen::Matrix4d Trl = pKF->GetRelativePoseTlr().matrix().cast<double>();
...
InvDepthPoint::InvDepthPoint(double _rho, double _u, double _v,
                             KeyFrame *pHostKF)
    : rho(_rho), u(_u), v(_v), fx(pHostKF->fx), fy(pHostKF->fy),
...
/**
 * @brief 用于计算与单目标（mono）观察相关的边的雅可比矩阵
 */
...
/**
  * @brief 初始化一个惯性边缘 (Edge
   Inertial)，用于在图优化中处理与IMU相关的测量数据。它通过构造函数接收预积分的IMU数据，并初始化边缘连接的多个顶点，设置重力向量和信息矩阵等重要参数，以便进行后续的优化和状态估计。
  * @param pInt IMU预积分数据
  * @param dt 时间步长
  * @note
   构造函数中，对IMU预积分数据进行了处理，将其转换为double类型，并计算其雅可比矩阵
*/
...
/**
 * @brief
 * 初始化EdgeInertialGS类的实例。它整合了IMU的预积分数据，设置连接的顶点数量，初始化重力向量，并生成一个稳定的信息矩阵以供后续优化使用
 * @param pInt IMU预积分数据
 * @param dt 时间步长
 * @note
 * 这个类通常用于基于IMU的数据进行状态估计，其关键在于如何将传感器数据有效地整合到图优化结构中，从而提高估计的准确性与稳定性。
 */
...
/**
 * @brief 计算绕给定三维轴（由 (x, y, z) 表示）的旋转对应的旋转矩阵
...
/**
 * @brief 计算绕给定三维轴（由 (x, y, z)
 * 表示）的旋转对应的旋转矩阵的右雅克比矩阵
 * @param R Rotation matrix
 * @return The right Jacobian of the rotation matrix
 */
