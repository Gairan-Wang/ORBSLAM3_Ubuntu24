...
KeyFrame::KeyFrame()
    : mnFrameId(0), mTimeStamp(0), mnGridCols(FRAME_GRID_COLS),
      ...
      mnNumberOfOpt(0), mnLoopQuery(0), mnLoopWords(0), mnRelocQuery(0),
      mnRelocWords(0), mnMergeQuery(0), mnMergeWords(0),
      mnPlaceRecognitionQuery(0), mnPlaceRecognitionWords(0),
      mPlaceRecognitionScore(0), mbCurrentPlaceRecognition(false),
      mnBAGlobalForKF(0), mnMergeCorrectedForKF(0), mnBALocalForMerge(0), fx(0),
      fy(0), cx(0), cy(0), invfx(0), invfy(0), mbf(0), mb(0), mThDepth(0), N(0),
      mvKeys(static_cast<std::vector<cv::KeyPoint>>(0)),
      mvKeysUn(static_cast<std::vector<cv::KeyPoint>>(0)),
      mvuRight(static_cast<std::vector<float>>(0)),
      mvDepth(static_cast<std::vector<float>>(0)), mnScaleLevels(0),
      ...
      mNextKF(static_cast<KeyFrame *>(NULL)), mbHasVelocity(false),
      mbFirstConnection(true), mpParent(NULL), mbNotErase(false),
      mbToBeErased(false), mbBad(false), mHalfBaseline(0), NLeft(0), NRight(0) {
}
...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB)
    : bImu(pMap->isImuInitialized()), mnFrameId(F.mnId),
      ...
      mnBAFixedForKF(0), mnNumberOfOpt(0), mnLoopQuery(0), mnLoopWords(0),
      mnRelocQuery(0), mnRelocWords(0), mnPlaceRecognitionQuery(0),
      mnPlaceRecognitionWords(0), mPlaceRecognitionScore(0),
      mbCurrentPlaceRecognition(false), mnBAGlobalForKF(0),
      mnMergeCorrectedForKF(0), mnBALocalForMerge(0), fx(F.fx), fy(F.fy),
      cx(F.cx), cy(F.cy), invfx(F.invfx), invfy(F.invfy), mbf(F.mbf), mb(F.mb),
      mThDepth(F.mThDepth), mDistCoef(F.mDistCoef), N(F.N), mvKeys(F.mvKeys),
      ...
      mnMaxX(F.mnMaxX), mnMaxY(F.mnMaxY), mPrevKF(NULL), mNextKF(NULL),
      mpImuPreintegrated(F.mpImuPreintegrated), mImuCalib(F.mImuCalib),
      mNameFile(F.mNameFile), mnDataset(F.mnDataset), mbHasVelocity(false),
      mTlr(F.GetRelativePoseTlr()), mTrl(F.GetRelativePoseTrl()),
      ...
      mHalfBaseline(F.mb / 2), mpMap(pMap), mK_(F.mK_), mpCamera(F.mpCamera),
      mpCamera2(F.mpCamera2), mvLeftToRightMatch(F.mvLeftToRightMatch),
      mvRightToLeftMatch(F.mvRightToLeftMatch), mvKeysRight(F.mvKeysRight),
...
/**
 * @brief
 * !设置关键帧的速度。通过传入一个三维速度向量，函数能够更新关键帧的速度信息并相应地设置一个状态标志
...
void KeyFrame::ChangeParent(KeyFrame *pKF) {
  std::unique_lock<std::mutex> lockCon(mMutexConnections);
  if (pKF == this) {
    std::cout << "ERROR: Change parent KF, the parent and child are the same KF"
              << std::endl;
...
void KeyFrame::PreSave(std::set<KeyFrame *> &spKF, std::set<MapPoint *> &spMP,
...
void KeyFrame::PostLoad(std::map<long unsigned int, KeyFrame *> &mpKFid,
  ...
  if (mnBackupIdCamera >= 0) {
    ...
    std::cout << "ERROR: There is not a main camera in KF " << mnId
              << std::endl;
  }
