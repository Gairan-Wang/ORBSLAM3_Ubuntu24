...
Map::Map()
    : mpFirstRegionKF(static_cast<KeyFrame *>(NULL)), mbFail(false),
      mbImuInitialized(false), mnMapChange(0), mnMapChangeNotified(0),
      mnMaxKFid(0), mnBigChangeIdx(0), mIsInUse(false), mHasTumbnail(false),
      mbBad(false), mbIsInertial(false), mbIMU_BA1(false), mbIMU_BA2(false) {
  mnId = nNextId++;
...
Map::Map(int initKFid)
    : mpFirstRegionKF(static_cast<KeyFrame *>(NULL)), mbFail(false),
      /*mnLastLoopKFid(initKFid),*/ mbImuInitialized(false), mnMapChange(0),
      mnMapChangeNotified(0), mnInitKFid(initKFid), mnMaxKFid(initKFid),
      mnBigChangeIdx(0), mIsInUse(false), mHasTumbnail(false), mbBad(false),
